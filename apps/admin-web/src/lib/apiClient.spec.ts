import { createAdminApiClient, type AuthLike } from './apiClient';
...
describe('createAdminApiClient', () => {
  ...
  it('attaches the current Firebase user\'s ID token to requests', async () => {
    ...
    const client = createAdminApiClient(auth);
  ...
  it('signs the user out when the backend responds 401 (unauthorized/expired session)', async () => {

...
**Status:** Original V1 scope contract. Current implementation status is tracked in [doc 21](21-current-implementation-status.md).  
...
Customer and studio-staff experiences will ship in one AutoDeck mobile app and route by authenticated role. The tables below describe role capabilities, not separate product brands or a permanent requirement for separate binaries. AutoDeck Admin remains a web application.

## V1 Scope — Customer Role

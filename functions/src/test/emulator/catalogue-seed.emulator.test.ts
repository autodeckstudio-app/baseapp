/**
 * Emulator tests for the retired catalogue import (Phase 4).
 *
...
describe("Catalogue seed — idempotent retired catalogue import", () => {
  beforeAll(async () => {
    await seedCatalogue(TEST_TENANT);
  });

  it("contains exactly 18 validated studio services", () => {
  ...
  it("prices match the validated studio price list exactly (paise)", async () => {
  ...
  it("calculateServicePrice works correctly against a seeded multi-day PPF service", async () => {
    const result = (await calculateServicePrice.run({
      ...
      auth: {
        uid: "test-cust",
        token: { role: "customer", tenantId: TEST_TENANT, studioId: null },
        rawToken: "test",
      },

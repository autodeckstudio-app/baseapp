...
export const addStaffMember = onCall({ region: "asia-south1", enforceAppCheck: shouldEnforceAppCheck() }, async (request) => {
  ...
  // Phase 5B P1-6 fix: everything from here until the Employee record is
  // durably written can fail (claims write, transient Firestore error).
  // Previously a failure here left a real Firebase Auth account with no
  // Employee record — an orphan that permanently blocks retry (the
  // already-exists check above rejects the same email forever) and needs
  // manual GCP console cleanup. Compensating delete: if anything after
  // createUser() fails, remove the just-created Auth user so the caller can
  // retry cleanly. No transaction/outbox mechanism spans Auth + Firestore in
  // this codebase, so a compensating action is the established pattern here
  // (matches deactivateStaffMember's Auth-then-Firestore ordering, made
  // safe by cleanup instead of by reordering — reordering isn't available
  // here since the Employee doc's id IS the Auth uid, generated by
  // createUser()).
  try {

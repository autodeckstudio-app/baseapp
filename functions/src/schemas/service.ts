...
const warrantyDurationUnitEnum = z.enum([
  "days",
  "months",
  "years",
  "lifetime",
...
export const vehicleCategoryPricingSchema = z
  .object({
    vehicleCategory: vehicleCategoryEnum,
    additionalPricePaise: z
      .number()
      .int()
      .min(0, "Price must be non-negative paise"),
...
export const createServiceSchema = z
  .object({
    ...
    basePrice: z
      .number()
      .int()
      .min(0, "Base price must be a non-negative integer (paise)"),
    currency: z.string().length(3).optional(),
    // Cap raised from 1440 (1 day) to 20160 (2 weeks) in Phase 4: the real
    // validated studio PPF catalogue includes genuine multi-day jobs (up to 4320 min /
...
export const getServiceCatalogueSchema = z
  .object({
    category: serviceCategoryEnum.optional(),
  })
  .strict();
...
export type GetServiceCatalogueInput = z.infer<
  typeof getServiceCatalogueSchema
>;
